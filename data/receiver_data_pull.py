...
#'Colorado State','BYU', 'Clemson','Georgia','Harvard','Hawaii','Idaho State','James Madison','LSU','Michigan','Oaklahoma','Ohio State','Oregon', 'Penn State', 'South Dakota', 'Texas', 'UCLA', 'Washington', 'Utah State', 'Wyoming', 'Air Force', 'Boise State','SMU','Tulane','UTSA', 'Florida State', 'Louisville', 'NC State', 'Georgia Tech'
...
for team in teams:
    for year in years:
        ...
        if not os.path.exists('college_reciever_data/'+team):
            os.makedirs('college_reciever_data/'+team)
            
        x.to_csv('college_reciever_data/'+team + '/cfbd_recieving_data_' + str(year) + '.csv', index=False)
